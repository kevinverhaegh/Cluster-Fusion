function output = CompareRetard2(env, delta, info, t)
tb = min(t);
te = max(t);
t = tb:((te-tb)/(1e5)):te;
acc = 1e-16;
...
for i=2:numel(t)
    if t(i) - (sqrt(rt(i-1,:)*rt(i-1,:)')/PhysConst.c) > t(round(0.1*numel(t)))
        break
    end
end
...
t0 = tb:((te-tb)/(1e5)):t(beginsim);
t1 = t(beginsim):1e-20:t(beginsim)+1e-16;
