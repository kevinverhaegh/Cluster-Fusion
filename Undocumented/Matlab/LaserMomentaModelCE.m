function pout = LaserMomentaModelCE(x, eta0, pz0)
...
pout = pz0 + (0.5/(PhysConst.me.*PhysConst.c)).*((PhysConst.e^2).*((envi.*sin(x)).^2 -2.*envi.*sin(x).*envi0.*sin(eta0) - (envi0.*sin(eta0)).^2));
