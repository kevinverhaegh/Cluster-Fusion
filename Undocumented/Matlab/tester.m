function output =tester(env, delta, tb, te)
...
vt = frac.*t.*(PhysConst.c/(te));
at = frac*PhysConst.c / te;
...
for i=beginsim+1:numel(t)
    ...
    n=0;
    while (((rtry - rtry2)*(rtry - rtry2)')/(rtry*rtry') > 0.05)
        n=n+1;
        rtry = rtry2;
        tr(i) = t(i) - ((sqrt(rtry*rtry'))/PhysConst.c);
        [~,itr(i)] = min(abs(tr(i) - t));
...
FcInst(i,:) =  (PhysConst.e^2 / (4 * pi * PhysConst.epsilon0))*(1/(rhelp(i,:)*rhelp(i,:)')) .* (rhelp(i,:)./sqrt(rhelp(i,:)*rhelp(i,:)') + 1/PhysConst.c^2 * ...
    cross(vhelp(i,:), cross(vhelp(i,:), rhelp(i,:)./sqrt(rhelp(i,:)*rhelp(i,:)'))));
end
